            ...
            <div className="mt-8 card p-5 max-w-sm">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-sm text-slate-300">
                  I confirm that I am <strong className="text-slate-100">18 years of age or older</strong>,
                  and I agree to the{' '}
    ...
    <div className="space-y-4">
      {items.map((it) => (
        <div key={it.title} className="card p-4">
          <p className="font-display font-semibold text-slate-100">{it.title}</p>
          <p className="text-sm text-slate-400 mt-1">{it.body}</p>
        </div>
      ))}
    </div>
  );
