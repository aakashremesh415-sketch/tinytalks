        ...
        {!imageVerified && (
          <div className="mt-6 space-y-8">
            <section>
              ...
              {otpMsg && <p className="text-sm text-slate-400 mt-2">{otpMsg}</p>}
              {devPreview && (
            ...
            <section>
              ...
              {!user?.ageEstimationPassed && (
                <form onSubmit={submitAgeEstimation} className="mt-3 space-y-2">
                  ...
                  />
                  <button className="btn-secondary" disabled={ageBusy}>{ageBusy ? 'Checking…' : 'Submit selfie'}</button>
                </form>
              )}
              {ageMsg && <p className="text-sm text-slate-400 mt-2">{ageMsg}</p>}
            </section>
...
function StepHeader({ n, title, done }) {
  return (
    <div className="flex items-center gap-3">
      <div className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${done ? 'bg-mint-500 text-ink-950' : 'bg-white/10 text-slate-300'}`}>
        {done ? '✓' : n}
      </div>
